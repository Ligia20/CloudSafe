...
import {
  ...
  IonCard,IonCardHeader,IonCardTitle,IonCardSubtitle,IonCardContent,IonText,
  IonList,IonItem,IonLabel,IonBadge,IonButton,IonIcon,useIonViewWillEnter
} from "@ionic/react";
import { bug,globe,lockClosed,person } from "ionicons/icons";
import {
...
const DashboardPage:React.FC=()=>{
  ...
  const clearMutation=useMutation({
    mutationFn:async()=>{
      ...
      if(!response.ok){
        ...
        throw new Error(
          errorData?.error||"Failed to clear old records"
        );
      }
  ...
  const simulateMutation=useMutation({
    mutationFn:async(attackType:string)=>{
      ...
      if(!response.ok){
        const errorData=await response.json().catch(()=>null);
        throw new Error(
          errorData?.error||"Simulation failed"
        );
      }
  ...
  const alertsQuery=useQuery({
    ...
    queryFn:async()=>{
      ...
      if(!response.ok){
        const errorData=await response.json().catch(()=>null);
        throw new Error(
          errorData?.error||"Failed to retrieve alerts"
        );
      }
  ...
  const recentQuery=useQuery({
    ...
    queryFn:async()=>{
      ...
      if(!response.ok){
        const errorData=await response.json().catch(()=>null);
        throw new Error(
          errorData?.error||"Failed to retrieve recent data"
        );
      }
  ...
  const investigatingAlerts=data.investigatingAlerts??0;
  const resolvedAlerts=data.resolvedAlerts??0;

  const recentEvents=recent.events||events.slice(0,10);
  const recentAlerts=recent.alerts||alerts.slice(0,10);
  ...
  const chartEvents=[
    {name:"Events",events:totalEvents}
  ];

  const chartRecentEvents=recentEvents.map(
              ...
              <IonCard>
                ...
                <IonCardContent>
                  <IonList>
                    ...
                    <IonItem>
                      <IonLabel>Total Events</IonLabel>
                      <IonBadge slot="end">
                        {totalEvents}
                      </IonBadge>
                    </IonItem>
                    ...
                    <IonItem>
                      <IonLabel>Total Assets</IonLabel>
                      <IonBadge slot="end">
                        {totalAssets}
                      </IonBadge>
                    </IonItem>
                    ...
                    <IonItem>
                      <IonLabel>Active Alerts</IonLabel>
                      <IonBadge
                        color="danger"
                        slot="end"
                      >
              ...
              <IonCard>
                ...
                <IonCardContent>
                  <IonButton
                    ...
                    onClick={()=>
                      simulateMutation.mutate("brute-force")
                    }
                  >
              ...
              <IonCard>
                ...
                <IonCardContent>
                  <IonButton
                    ...
                    disabled={simulateMutation.isPending}
                    onClick={()=>
                      simulateMutation.mutate("port-scan")
                    }
                  >
                    {simulateMutation.isPending
                      ? "Simulating..."
              ...
              <IonCard>
                ...
                <IonCardContent>
                  <IonButton
                    ...
                    disabled={simulateMutation.isPending}
                    onClick={()=>
                      simulateMutation.mutate("malware")
                    }
                  >
                    {simulateMutation.isPending
                      ? "Simulating..."
